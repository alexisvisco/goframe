...
The `tsclient` generator produces a typed client for your HTTP API. It automatically discovers all handler packages in a hierarchical structure, reads route declarations and builds functions that validate requests and responses using [Zod](https://github.com/colinhacks/zod).
...
1. **Root Package Handlers**: Use handler name directly
   ```ts
   // internal/v1handler/handler_user.go → UserHandler
...
2. **Subpackage Handlers**: Include package path as prefix
   ```ts
   // internal/v1handler/dashboard/handler_stats.go → StatsHandler
   export namespace DashboardStatsClient {
     export async function getStats(request: GetStatsRequest){}
   }

   // internal/v1handler/admin/reports/handler_reports.go → ReportsHandler
   export namespace AdminReportsReportsClient {
     export async function getReports(request: GetReportsRequest){}
   }
   ```

3. **Multiple Root Packages**: Separate files generated
   ```ts
   // internal/v1handler generates one file
   // internal/v2handler generates another file
   ```
...
4. **Type Prefixing**: Types are prefixed when conflicts occur between packages
   ...
   export namespace DashboardUserClient {
     export async function getCompanies(request: DashboardGetCompaniesRequest){}
   }
   ```
