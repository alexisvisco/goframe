...
The IP middleware extracts the real client IP address from proxy headers and sets it as `r.RemoteAddr`. This is essential when your application runs behind load balancers, CDNs, or reverse proxies that add IP information to request headers.
