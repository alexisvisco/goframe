...
The generator resolves naming conflicts across handler packages. Types from sub-packages are automatically prefixed when a root package defines a type with the same name. Namespaces always include the full package path to keep the hierarchy clear.
...
Nested packages concatenate their segments (`admin/reports` → `AdminReports`), and external packages receive a prefix only when their type names clash with local ones.
