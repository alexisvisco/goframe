...
```
Note that is you try to create two schedules with the same ID, Temporal will return an error. You can handle this by checking for the `temporalutil.IsScheduleAlreadyExistError` error, which is a utility function to check if the error is due to an existing schedule.

I agree with you that this is quite verbose but you can create helpers on your side to simplify a little bit the code.
